...
async function searchSuperhero() {
    ...
    try {
        if (filterType === 'Power') {
            ...
            const url = `http://${hostNm}:8080/api/superhero_powers/byPower?power=${encodeURIComponent(selectedPower)}` + (count ? `&n=${count}` : '');
            ...
            const url = `http://${hostNm}:8080/api/superhero_info/match?field=${encodeURIComponent(filterType)}&pattern=${encodeURIComponent(searchTerm)}` + (count ? `&n=${count}` : '');
            const response = await fetch(url);
...
async function populatePowerDropdown() {
    const powerDropdown = document.getElementById('powerDropdown');
    const response = await fetch(`http://${hostNm}:8080/api/superhero_powers/all`);
...
async function createNewList() {
    ...
    try {
        const response = await fetch(`http://${hostNm}:8080/api/superhero_lists/`, {
            method: 'POST',
            headers: {
...
async function populateListsDropdown() {
    ...
    const response = await fetch(`http://${hostNm}:8080/api/superhero_lists/listNames`);
...
async function displayListDetails() {
    ...
    try {
      // Fetch the detailed information of superheroes in the selected list
        const response = await fetch(`http://${hostNm}:8080/api/superhero_lists/${encodeURIComponent(listName)}/info`);
      if (!response.ok) {
        throw new Error("There was a problem with the fetch operation.");
      }
...
async function populateSuperheroNamesDropdown() {
    ...
    const response = await fetch(`http://${hostNm}:8080/api/superhero_info/allNames`);
...
async function addSuperheroesToList() {
    ...
    try {
        ...
        const idsResponse = await fetch(`http://${hostNm}:8080/api/superhero_info/getIDs`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const listResponse = await fetch(`http://${hostNm}:8080/api/superhero_lists/${encodeURIComponent(listName)}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function deleteList() {
    ...
    try {
        const response = await fetch(`http://${hostNm}:8080/api/superhero_lists/${encodeURIComponent(listName)}`, {
            method: 'DELETE',
        });
